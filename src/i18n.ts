...
export const T: Record<Lang, Dict> = {
  es: {
    ...
    histFromChain: "traído de la cadena",
    histRebuild: "rearmar comprobante",
    histRebuildTitle: "Rearmá el comprobante",
    histRebuildNote: "Los nombres nunca subieron a la cadena, solo su huella. Pegá la lista de ese sorteo y, si es la misma, la huella coincide y el comprobante vuelve a salir.",
    histRebuildGo: "Rearmar",
    histRebuildOk: "Listo:",
    histRebuildBad: "Esa no es la lista que se selló: la huella no coincide.",
    histRebuildCount: "Pegaste {n} nombres y ese sorteo tenía {m}.",
    histWinnerAt: "nº {n} de la lista",
    ...
    cTelCount: (n, cabs) => (cabs > 1 ? `¡${n} pasajeros en ${cabs} cabinas!` : `¡${n} pasajero, una cabina!`),
    cTelGo: ["¡ARRANCA EL TELEFÉRICO!", "¡Para arriba, señores!", "¡Se despega del piso!"],
    cTelPass: (k) => pick([`¡Pasa de largo por la ${k}!`, `¡En la ${k} no se baja nadie!`, `¡La ${k}, de largo!`]),
    cTelStop: (k, off, left) => (off === 1 ? `¡Estación ${k}! Se baja uno, quedan ${left}` : `¡Estación ${k}! Se bajan ${off}, quedan ${left}`),
    cTelUp: ["¡Sigue subiendo!", "¡Más arriba todavía!", "¡Qué vista, señores!"],
    cTelLast: ["¡QUEDAN DOS!", "¡Última estación!", "¡Acá se baja uno!"],
    cTelOff: (n) => pick([`¡Se baja ${n}!`, `¡${n} se queda en el andén!`, `¡Hasta acá llegó ${n}!`]),
    cTelClimb: ["¡Sube solita!", "¡Ya casi llega!", "¡Nadie respira!"],
    ...
    cTomSpin2: ["¡Otra vuelta!", "¡Más fuerte!", "¡Que se mezclen bien!"],
    cTomSpin3: ["¡LA ÚLTIMA VUELTA!", "¡A TODA MÁQUINA!", "¡Nadie la toca!"],
    cTomStop: ["Se frena…", "Silencio, señores…", "Ya casi…"],
    cTomDoor: ["¡SE ABRE LA PUERTA!", "¡Sale una!", "¡Ahí viene!"],
    cTomBall: (num) => pick([`¡La número ${num}!`, `¡Salió la ${num}!`, `¡Es la ${num}, señores!`]),
    cTomRoll: ["¡Baja, baja!", "¡Viene rodando!", "¡No se detiene!"],
    ...
    loreTelefericoQ: "¿Por qué cada cabina tiene un color?",
    loreTelefericoA: "Porque así se llaman las líneas de Mi Teleférico, que une La Paz con El Alto: Roja, Amarilla, Verde, Azul y seis más. Son diez líneas y 1.396 cabinas de diez pasajeros, la red de teleféricos urbanos más grande del mundo según la empresa que la construyó.",
    ...
    cConstBuild: ["¡Se arma el cielo!", "¡Miren arriba, señores!", "¡Se prenden las estrellas!"],
    ...
    waitTitle: "La ronda que va a decidir",
    waitLine: "Todavía no existe. Nace sola, en la red del faro, y ni vos ni nadie puede saber qué número va a traer. Por eso el sorteo ya no se puede acomodar.",
    ...
    // --- Las páginas de lectura: juegos, historia y seguridad. Viven aparte de
    //     la herramienta: quien entra a sortear no tiene que leer nada antes.
    masTitle: "Para saber más",
    masJuegosT: "Los ocho juegos",
    masJuegosB: "Carrera de llamas, ruleta, pasanaku, teleférico, tómbola y tres más, con la captura de cada uno.",
    ...
    navJuegos: "Juegos", navHistoria: "Historia", navSeguridad: "Seguridad",
    navPrecios: "Precios", navCodigo: "Código",
    jgKicker: "Ocho juegos",
    jgTitle: "El sorteo se mira",
    jgLead: "Cuando arranca la animación, el ganador ya está decidido: salió del número público del faro. El juego solo lo cuenta. Cada uno pasa veinte comprobaciones antes de entrar, y la que importa es que el nombre en pantalla sea el que fijó el protocolo.",
    jgRaceN: "Carrera de llamas",
    jgRaceB: "Lo nuestro. Ocho carriles por la cordillera y una llama por persona. Hasta ocho en pantalla.",
    jgWheelN: "Ruleta",
    jgWheelB: "La de siempre, con el perno tricolor marcando la vuelta. Los gajos se reparten intercalados, así que con dos personas igual se ve girar. Hasta veinticuatro.",
    jgStellarN: "Constelación Stellar",
    jgStellarB: "Un pago que salta de estrella en estrella buscando ruta, como un path payment. Deja dibujada una constelación. Hasta doscientas.",
    jgLedgerN: "Cierre de Libro",
    jgLedgerB: "Tarjetas barridas por el cierre de un ledger hasta que queda una sellada. Dura lo que tarda Stellar en cerrar uno. Hasta doscientas.",
    jgPasN: "Pasanaku",
    jgPasB: "El ahorro rotativo boliviano: un aguayo que se cierra sobre los bultos hasta que queda uno en el nudo. Los hilos entre vecinos son trustlines. Hasta doscientas.",
    jgTelN: "Teleférico",
    jgTelB: "Un convoy de cabinas sube por el cable y en cada estación se baja la mitad, hasta que una sola llega a la cumbre y abre la puerta. Las cabinas llevan los colores de las líneas de La Paz y El Alto. Hasta doscientas.",
    jgTomN: "Tómbola",
    jgTomB: "El bombo de la kermés: una bola por persona, con su número en la lista sellada. Tres vueltas de manivela, se abre la compuerta y sale una sola, que baja rodando hasta el vaso. Hasta doscientas.",
    jgRocketsN: "Carrera de cohetes",
    ...
    hiAguayoB: "En el juego del Pasanaku, la tela no es un fondo bonito: es donde pasa el juego. Los bultos se acomodan sobre las franjas y el nudo se cierra sobre el que queda. El pallay, esas bandas claras con rombos, es lo que hace que una tela se lea como aguayo y no como cinco colores apilados.",
    hiNombresT: "Quispe, Mamani, Choque",
    hiNombresB: "La lista de ejemplo está llena de apellidos que en Bolivia son los de siempre: Quispe, Mamani, Choque, Condori, Limachi, Villca. Es el detalle más chico del producto y el que más gusta, porque cualquiera que abra la demo se reconoce en la lista.",
    hiPasanakuT: "El pasanaku existe hace rato",
    hiPasanakuB: "Un grupo pone plata cada mes y cada vez le toca a uno, por sorteo o por turno. Funciona porque todos se conocen y nadie quiere quedar mal. Tinkazo es eso mismo cuando el grupo ya no se conoce: en vez de confianza entre vecinos, una cuenta que cualquiera puede rehacer.",
    hiReglaT: "Lo boliviano entra por la función",
    ...
    sgQ3B: "Cualquiera puede finalizar un sorteo sellado: la función que registra el resultado no pide permiso a nadie. Si al organizador se le corta la luz a mitad del evento, alguien más abre el comprobante y lo cierra, y el ganador es el mismo.",
    sgQ4T: "Cambiar el contrato",
    sgQ4B: "El contrato es inmutable: no tiene administrador ni forma de actualizarse, y no custodia fondos. Una versión nueva es una dirección nueva, anotada en la documentación con su fecha.",
    sgOpenT: "El ataque que sigue abierto",
    sgOpenK: "selección del compromiso",
    sgOpenB: "Un organizador puede sellar la misma lista cinco veces contra cinco rondas distintas y publicar solo el comprobante que le convino. Cada uno de esos sorteos es, por separado, perfectamente legítimo. Esta es la única forma conocida de torcer un sorteo de Tinkazo y no está cerrada.",
    sgOpenD: "<b>Lo que hay contra eso:</b> la página de verificación trae todos los sellos de esa dirección, leídos del contrato, y marca sola los que repiten la huella de la lista o la cantidad de gente. Si selló cinco, se ven las cinco. Cerrarlo del todo exigiría que el organizador anuncie el identificador del sorteo por un canal que él no controle, y eso Tinkazo no lo puede imponer.",
    sgLimitT: "Los límites que hay que decir",
    sgLimit1: "Sin anclar en Stellar, la cuenta cierra igual, pero nadie más que el organizador atestigua que esa era la lista. La página lo dice con un veredicto amarillo en vez de un verde fácil.",
    sgLimit2: "Los nombres nunca suben a la cadena, solo su huella. Eso protege a los participantes y, a la vez, hace que el comprobante dependa de que el organizador conserve la lista.",
    sgLimit3: "Lo que el contrato guarda tiene alquiler: si nadie lo extiende, la entrada se archiva y deja de leerse hasta que alguien la restaure. El sorteo no se pierde, pero hay que pagar para volver a verlo.",
    sgLimit4: "Si el faro público se cayera entero y para siempre, un sorteo sellado quedaría sin número. Son cuatro relays independientes y se consultan todos.",
    sgReportT: "Si encontrás una forma de arreglar un sorteo",
    sgReportB: "Abrí un issue. Es el reporte que más sirve, y el proyecto es código abierto justamente para eso: el protocolo completo, el contrato y las amenazas conocidas están escritos y se pueden auditar sin pedirle permiso a nadie.",
    sgLinkProto: "El protocolo ↗", sgLinkAmen: "Las amenazas ↗", sgLinkIssue: "Abrir un issue ↗",
    ...
    vSealsWhy: "Un organizador podría sellar la misma lista varias veces y publicar solo el resultado que le conviene. Todos los sellos son públicos, así que acá están los suyos, todos los que siguen en la cadena. Ninguno repite la lista de este sorteo ni su cantidad de gente.",
    vSealsSameCount: "misma cantidad de gente",
    vSealsSameCountWhy: "Hay otros sellos con la misma cantidad de gente que este. Puede ser otro sorteo del mismo evento, o la misma lista con los nombres en otro orden, que da otra huella. Si están cerca en el tiempo, pedile al organizador los comprobantes.",
    vSealsOnly: "Este organizador no selló ninguna otra lista.",
    vSealsAll: "todos los que siguen en la cadena",
    vSealsMany: "hay más de los que entran acá",
    vSealsAgain: "la misma lista, sellada otra vez",
    vSealsAgainWhy: "Esta misma lista se selló más de una vez. Puede tener una explicación, como un sorteo que se trabó y se repitió, pero también es la forma de probar suerte con varias rondas y publicar solo el resultado que conviene. Pedile al organizador los comprobantes de todos.",
  ...
  en: {
    ...
    histWhy: "This device remembers the raffles you ran here, names included. The ones you anchored from another device come from the chain: the date, the prize and which spot on the list won are there, but not the names, which are never published. Clearing the site data loses this device's list, not the raffles, and every receipt keeps working forever from anywhere.",
    histFromChain: "from the chain",
    histRebuild: "rebuild receipt",
    histRebuildTitle: "Rebuild the receipt",
    histRebuildNote: "The names never went on chain, only their fingerprint. Paste that raffle's list and, if it is the same one, the fingerprint matches and the receipt comes back.",
    histRebuildGo: "Rebuild",
    histRebuildOk: "Done:",
    histRebuildBad: "That is not the list that was sealed: the fingerprint does not match.",
    histRebuildCount: "You pasted {n} names and that raffle had {m}.",
    histWinnerAt: "#{n} on the list",
    ...
    cTelCount: (n, cabs) => (cabs > 1 ? `${n} riders in ${cabs} cabins!` : `${n} rider, one cabin!`),
    cTelGo: ["THE CABLE CAR IS OFF!", "Up we go, people!", "Off the ground!"],
    cTelPass: (k) => pick([`Straight through station ${k}!`, `Nobody gets off at ${k}!`, `Station ${k}, no stop!`]),
    cTelStop: (k, off, left) => (off === 1 ? `Station ${k}! One gets off, ${left} left` : `Station ${k}! ${off} get off, ${left} left`),
    cTelUp: ["Still climbing!", "Higher still!", "What a view, people!"],
    ...
    cTomSpin2: ["Another spin!", "Harder!", "Mix them well!"],
    cTomSpin3: ["LAST SPIN!", "FULL SPEED!", "Hands off!"],
    cTomStop: ["It's slowing…", "Quiet, people…", "Almost…"],
    cTomDoor: ["THE DOOR OPENS!", "One's coming out!", "Here it comes!"],
    cTomBall: (num) => pick([`Number ${num}!`, `Out comes ${num}!`, `It's ${num}, people!`]),
    cTomRoll: ["Down it goes!", "Rolling, rolling!", "It won't stop!"],
    ...
    loreTelefericoQ: "Why does each cabin have a color?",
    loreTelefericoA: "Because that is how Mi Teleférico, linking La Paz and El Alto, names its lines: Red, Yellow, Green, Blue and six more. Ten lines and 1,396 ten-passenger cabins, the world's biggest urban ropeway network according to the company that built it.",
    ...
    cConstBuild: ["The sky is coming together!", "Look up, people!", "Stars switching on!"],
    ...
    waitTitle: "The round that will decide",
    waitLine: "It does not exist yet. It is born on its own, out on the beacon network, and nobody ·not even you· can know what number it will bring. That is why this draw can no longer be arranged.",
    ...
    masJuegosT: "The eight games",
    masJuegosB: "Llama race, wheel, pasanaku, cable car, raffle drum and three more, with a shot of each one.",
    masHistoriaT: "Where the name comes from",
    masHistoriaB: "A tinkazo is a hunch and a tincazo is a flick. A raffle is both.",
    ...
    masPreciosB: "Four cents per anchored raffle. Checking is free, always.",
    navJuegos: "Games", navHistoria: "Story", navSeguridad: "Security",
    navPrecios: "Pricing", navCodigo: "Code",
    jgKicker: "Eight games",
    jgTitle: "The draw is watched",
    jgLead: "When the animation starts, the winner is already decided: it came out of the beacon's public number. The game only tells it. Each one passes twenty checks before it ships, and the one that matters is that the name on screen is the one the protocol set.",
    jgRaceN: "Llama race",
    jgRaceB: "Ours. Eight lanes across the mountains, one llama per person. Up to eight on screen.",
    jgWheelN: "Wheel",
    jgWheelB: "The usual one, with the tricolor pin marking the turn. Slices are interleaved, so it still spins properly with two people. Up to twenty-four.",
    jgStellarN: "Stellar constellation",
    jgStellarB: "A payment hopping from star to star looking for a route, like a path payment. It leaves a constellation drawn. Up to two hundred.",
    jgLedgerN: "Ledger close",
    jgLedgerB: "Cards swept by a ledger close until one is left sealed. It lasts what Stellar takes to close one. Up to two hundred.",
    jgPasN: "Pasanaku",
    jgPasB: "The Bolivian rotating savings circle: an aguayo closing over the bundles until one is left in the knot. The threads between neighbors are trustlines. Up to two hundred.",
    jgTelN: "Cable car",
    jgTelB: "A convoy of cabins climbs the cable and half the riders get off at every station, until a single cabin reaches the top and opens its doors. The cabins wear the colors of the La Paz and El Alto lines. Up to two hundred.",
    jgTomN: "Raffle drum",
    jgTomB: "The fair's raffle drum: one ball per person, carrying their number on the sealed list. Three turns of the crank, the door opens and a single ball comes out and rolls down into the cup. Up to two hundred.",
    jgRocketsN: "Rocket race",
    ...
    hiLead: "In Bolivia, a tinkazo is that feeling that today is your lucky day. And a tincazo, with a c, is the flick you give sliding a finger off your thumb. Both are in the Diccionario de americanismos. A raffle is both: the hunch and the flick that decides.",
    ...
    hiAguayoB: "In the Pasanaku game the cloth is not a pretty background: it is where the game happens. The bundles sit on its bands and the knot closes over the one left. The pallay, those light bands with diamonds, is what makes a cloth read as an aguayo instead of five stacked colors.",
    hiNombresT: "Quispe, Mamani, Choque",
    hiNombresB: "The sample list is full of surnames that in Bolivia are the everyday ones: Quispe, Mamani, Choque, Condori, Limachi, Villca. It is the smallest detail in the product and the best liked, because anyone opening the demo recognizes themselves in the list.",
    hiPasanakuT: "The pasanaku has been around",
    hiPasanakuB: "A group puts in money every month and each time it goes to one of them, by draw or by turn. It works because everyone knows each other and nobody wants to look bad. Tinkazo is that same thing when the group no longer knows each other: instead of trust between neighbors, math anyone can redo.",
    hiReglaT: "The Bolivian part earns its place",
    ...
    sgQ2T: "Picking the number that decides",
    sgQ2B: "No either. The number is published by drand, a public randomness beacon run by Cloudflare, Protocol Labs, EPFL and others, and the round that will decide is fixed before that round exists. The contract requires at least thirty seconds of margin.",
    sgQ3T: "Holding back a result you did not like",
    sgQ3B: "Anyone can finish a sealed raffle: the function that records the result asks nobody for permission. If the organizer loses power mid-event, someone else opens the receipt and closes it, and the winner is the same.",
    sgQ4T: "Changing the contract",
    sgQ4B: "The contract is immutable: no admin, no upgrade path, and it holds no funds. A new version is a new address, written down in the docs with its date.",
    sgOpenT: "The attack still open",
    sgOpenK: "commitment selection",
    sgOpenB: "An organizer can seal the same list five times against five different rounds and publish only the receipt that suited them. Each of those raffles is, on its own, perfectly legitimate. This is the only known way to rig a Tinkazo raffle and it is not closed.",
    sgOpenD: "<b>What stands against it:</b> the verification page brings every seal from that address, read from the contract, and flags on its own the ones that repeat the list fingerprint or the head count. If they sealed five, all five show. Closing it for good would require the organizer to announce the raffle id through a channel they do not control, and Tinkazo cannot impose that.",
    sgLimitT: "The limits worth saying",
    sgLimit1: "Without anchoring on Stellar the math still adds up, but nobody other than the organizer witnesses that this was the list. The page says so with a yellow verdict instead of an easy green.",
    sgLimit2: "Names never go on chain, only their fingerprint. That protects participants and, at the same time, makes the receipt depend on the organizer keeping the list.",
    sgLimit3: "What the contract stores pays rent: if nobody extends it, the entry is archived and stops being readable until someone restores it. The raffle is not lost, but seeing it again costs.",
    sgLimit4: "If the public beacon went down entirely and forever, a sealed raffle would be left without its number. There are four independent relays and all of them are queried.",
    sgReportT: "If you find a way to rig a raffle",
    sgReportB: "Open an issue. That is the most useful report there is, and the project is open source precisely for that: the full protocol, the contract and the known threats are written down and can be audited without asking anyone.",
    sgLinkProto: "The protocol ↗", sgLinkAmen: "The threats ↗", sgLinkIssue: "Open an issue ↗",
    ...
    vSealsWhy: "An organizer could seal the same list several times and publish only the convenient result. Every seal is public, so here are theirs, all the ones still on chain. None of them repeats this raffle's list or its head count.",
    vSealsSameCount: "same number of people",
    vSealsSameCountWhy: "There are other seals with the same number of people as this one. It may be another raffle at the same event, or the same list with the names in another order, which gives a different fingerprint. If they are close in time, ask the organizer for the receipts.",
    vSealsOnly: "This organizer sealed no other list.",
    vSealsAll: "all still on chain",
    vSealsMany: "there are more than fit here",
    vSealsAgain: "the same list, sealed again",
    vSealsAgainWhy: "This same list was sealed more than once. There may be a reason, like a raffle that got stuck and was redone, but it is also how you try your luck with several rounds and publish only the convenient result. Ask the organizer for the receipts of all of them.",
